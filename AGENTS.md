...
This repository turns Axios Pro Rata emails into a weekly structured deal dataset, a market review, and a Gmail draft for Esther.
...
10. Create a Gmail draft to `esther330825@gmail.com`. Never send it. Use the week and date range in the subject and include the complete review in the body.
11. Run final checks, then commit only the new or intentionally corrected weekly data, review, and relevant workflow files. Push the current fixed automation branch. Do not create a new per-run branch.

## Failure behavior

- If Gmail or GitHub authorization is unavailable, stop and report the exact missing connection.
...
- Never send email, merge a pull request, rewrite Git history, or delete historical branches as part of the weekly run.
